...
enum class SubjectName : int {
	...
	Fysica = 4, 
	Nederlands = 5, 
	LichaamelijkeOpvoeding = 6 // we use integers to not have problems if we change syntax...
};
	...
	: QMainWindow(parent)
{
	...
	if (db.open()) {
		...
		query.exec("CREATE TABLE users (Username TEXT, Password TEXT, Role TEXT, Name TEXT, Surname TEXT, DateOfBirth TEXT)"); // users
		query.exec("CREATE TABLE subjects (SubjectID INTEGER PRIMARY KEY, SubjectName TEXT, TeacherEmail TEXT)"); // subject has a teacher
		query.exec("CREATE TABLE registrations (StudentEmail TEXT, SubjectID INTEGER)"); // store students of a class

		vector<User*> gebruikers = { &teacher1, &teacher2, &student1, &student2, &student3 };

		for (User* u : gebruikers) {
			query.prepare("INSERT INTO users (Username, Password, Role, Name, Surname, DateOfBirth) VALUES (:user, :pass, :role, :name, :surname, :dob)");
			...
			query.bindValue(":surname", QString::fromStdString(u->getSurname()));

			Date d = u->getDateOfBirth();             // Get your 3 ints
			QDate qdob(d.year, d.month, d.day);       // Make it a "Smart Date"
			QString storageDate = qdob.toString(Qt::ISODate); // make it string to store
			query.bindValue(":dob", storageDate);    
			...
			if (Teacher* t = dynamic_cast<Teacher*>(u)) {
				...
				for (Subject* sub : alleVakkenLijst) {
					if (sub->getTeacher() == t) {
						QSqlQuery subQuery;
						subQuery.prepare("INSERT INTO subjects (SubjectID, Subjectname, TeacherEmail) VALUES (:id, :name,:teacher) ");
						subQuery.bindValue(":id", static_cast<int>(sub->getName()));
						subQuery.bindValue(":name", QString::fromStdString(subjectToString(sub->getName())));
						subQuery.bindValue(":teacher", QString::fromStdString(t->getEmail()));
						subQuery.exec();
					}
				}
				...
				for (const Subject& sub : s->getSubjects()) {
					QSqlQuery regQuery;
					regQuery.prepare("INSERT INTO registrations (StudentEmail, SubjectID) VALUES (:email,:id)");
					regQuery.bindValue(":email", QString::fromStdString(s->getEmail()));
					regQuery.bindValue(":id", static_cast<int>(sub.getName()));
					regQuery.exec();
				}
			}
			else {
...
void SystemProgrammingv2::on_btnLogin_clicked() {
	...
	QSqlQuery query;
	query.prepare("SELECT Role, Name, Surname, DateOfBirth FROM users WHERE Username = :user AND Password = :pass");
	...
	if (query.exec() && query.next()) {
		...
		QString DateOfBirth = query.value(3).toString();

		if (role == "Leerling") {
			QSqlQuery subQuery;
			// we have two tables, so we have to align them (join)
			subQuery.prepare("SELECT subjects.SubjectName " // take the name of the class (course)
				"FROM registrations " // look there
				"JOIN subjects ON registrations.SubjectID = subjects.SubjectID " // it's the link!, JOIN = bring subjects table
				"WHERE registrations.StudentEmail = :email"); // of the mail we want
			...
			ui.lblStatus->setText("Welkom " + RealName + " " + Surname + "\nBorn on " + DateOfBirth + " !\nIngeschreven voor: " + vakkenStr);
		}
